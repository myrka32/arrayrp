...
mp.events.add("playerJoin", (player) => {
    player.colorForOverlayIdx = function(index) {
        ...
        gm.mysql.handle.query("SELECT money FROM accounts WHERE username=? LIMIT 1", [player.name], (err, rows) => {
            if (err) {
                console.log(eventErrorPre + err.message);
            } else {
                if (rows.length > 0) {
                    player.data.customMoney = rows[0].Money;
                } else {
                    gm.mysql.handle.query("INSERT INTO accounts SET username=?, money=?", [player.name, startingMoney], (regErr) => {
                        if (regErr) {
                            console.log(eventErrorPre + regErr.message);
                        } else {
                            player.data.customMoney = startingMoney;
                        }
                    });
                }
            }
    ...
    player.sendToWorld = function() {
        player.position = new mp.Vector3(217.86, -334.77, 45.68);
        player.heading = 313.65;
...
mp.events.addCommand("creator", (player) => {
    ...
    if (freemodeCharacters.indexOf(player.model) == -1) {
        ...
        player.outputChatBox("!{red}Вы не можете поменять персонажа находясь в машине.");
    } else {
